...
impl Widget for &AnimationPhase {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let (glyph, emoji_positions, accent_mod) = if w >= 82 {
            full_omega(self)
        } else {
            compact_omega(self)
        };
        ...
        // Render orbiting cooking emoji
        for (dx, dy, emoji_char) in emoji_positions {
            ...
            if y < area.bottom() && x < area.right() && x >= area.left() {
                if let Some(cell) = theme::buf_cell_mut(buf, x, y) {
                    cell.set_char(emoji_char);
                    cell.set_style(accent);
                }
            }
        }

        // Tagline below the glyph
        if w >= 60 && self.agent == AgentState::Idle {
            let tagline = "Your AI sous-chef for code";
            let tagline_x = start_x + (glyph_width.saturating_sub(tagline.len() as u16)) / 2;
            let tagline_y = start_y + glyph_height + 1;
            if tagline_y < area.bottom() {
                for (i, ch) in tagline.chars().enumerate() {
                    let x = tagline_x + i as u16;
                    if x < area.right() {
                        if let Some(cell) = theme::buf_cell_mut(buf, x, tagline_y) {
                            cell.set_char(ch);
                            cell.set_style(theme::style_dim());
                        }
                    }
                }
            }
...
fn full_omega(phase: &AnimationPhase) -> (Vec<&'static str>, Vec<(i16, i16, char)>, Option<Modifier>) {
    ...
    // Cooking emoji orbiting at six positions, rotating with tick
    ...
    // Select emoji based on tick for variety
    let emoji_set = if phase.agent == AgentState::Streaming {
    ...
    let emoji_positions: Vec<(i16, i16, char)> = (0..6)
        .map(|i| {
    ...
    (glyph, emoji_positions, modifier)
}
...
fn compact_omega(phase: &AnimationPhase) -> (Vec<&'static str>, Vec<(i16, i16, char)>, Option<Modifier>) {
    ...
    (glyph, emoji_positions, accent_mod)
}