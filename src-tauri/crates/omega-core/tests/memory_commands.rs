...
async fn test_memory_store_and_search() {
    ...
    let result = memory::memory_search_project(&state, "test".into())
        .await
        .unwrap();
...
async fn test_memory_list() {
    let state = omega_core::AppState::new(":memory:");
    memory::memory_store_project(&state, "key1".into(), "val1".into())
        .await
        .unwrap();
    memory::memory_store_project(&state, "key2".into(), "val2".into())
        .await
        .unwrap();
